...
Date: 2026-06-14
Decision record: ADR-0039. Baseline and method: `00-baseline.md`. Findings: `01-findings.md`.
...
| F-003 OpenTofu unconfined `chdir` | Low | Fixed + test (re-add: BL-104) | `actuation/opentofu.py` |
...
Deferred hardening: BL-104 (OpenTofu `chdir` confinement), BL-105 (timing-safe approval
comparison), BL-106 (multi-client message-byte cap), BL-107 (probe collector caps),
BL-108 (exhaustive compliance proving-tests). BL-105 and BL-106 are HTTP-transport
...
typo). ADR README index and prose extended for ADR-0039.
...
The remediations and documentation updates ship in one change with ADR-0039 and these
...
items (especially BL-105/106) gate the HTTP transport.
